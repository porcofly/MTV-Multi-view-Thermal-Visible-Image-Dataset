...
def compute_symmetrical_epipolar_errors(data):
    ...
    """
    # a = data['T_0to1'][:, :3, 3]
    # print("data['T_0to1'][:, :3, 3] :" ,data['T_0to1'][:, :3, 3])
    # print("data['T_0to1'][:, :3, :3] : ",data['T_0to1'][:, :3, :3])
    ...
    # print('mean epi_errs :' ,around(torch.mean(epi_errs.cpu() ),5))
    data.update({'epi_errs': epi_errs})


def estimate_pose_from_H(kpts0, kpts1, K0, K1, thresh, conf=0.99999):
    if len(kpts0) < 5:
        return None
    # normalize keypoints
    kpts0, kpts1 = normalize_coordinates_camera(kpts0, kpts1, K0, K1)
    kpts0 = np.array(kpts0,dtype=int)
    kpts1 = np.array(kpts1,dtype=int)    
    ...
    H, mask = cv2.findHomography(
        kpts0, kpts1, np.eye(3), threshold=ransac_thr,method=cv2.RANSAC)
    inliers = np.sum(mask)
...
def estimate_pose_my(kpts0, kpts1, K0, K1, thresh, conf=0.99999):
    if len(kpts0) < 5:
        return None
    # normalize keypoints
    # print(' K0[[0, 1], [2, 2]][None]:', K0[[0, 1], [2, 2]][None])
    # print('  K0[[0, 1], [0, 1]][None]:',  K0[[0, 1], [0, 1]][None])
    # print(' K1[[0, 1], [2, 2]][None]:', K1[[0, 1], [2, 2]][None])
    # print(' K1[[0, 1], [0, 1]][None]:', K1[[0, 1], [0, 1]][None])
    # print('kpts0:',kpts0[0:3])
    # print('kpts1:',kpts1[0:3])
    ...
    for _E in np.split(E, len(E) / 3):
        n, R, t, _ = cv2.recoverPose(_E, kpts0, kpts1, np.eye(3), mask=mask)
...
def estimate_pose(kpts0, kpts1, K0, K1, thresh, conf=0.99999):
    ...
    # print('thresh',thresh)
    ransac_thr = thresh / np.mean([K0[0, 0], K1[1, 1], K0[0, 0], K1[1, 1]])
    # print('ransac_thr',ransac_thr)
    # np.mean([K0[0, 0], K1[1, 1], K0[0, 0], K1[1, 1]])
    # compute pose with cv2
    E, mask = cv2.findEssentialMat(
        kpts0, kpts1, np.eye(3), threshold=ransac_thr, prob=conf, method=cv2.RANSAC)
    if E is None:
...
def compute_pose_errors(data, config):
    ...
    for bs in range(K0.shape[0]):
        ...
        # h_R,h_T = estimate_pose_from_H(pts0[mask], pts1[mask], K0[bs], K1[bs], pixel_thr, conf=conf)
        ...
        if ret is None:
            ...
            # h_t_err, h_R_err = relative_pose_error(T_0to1[bs], h_R,h_T, ignore_gt_t_thr=0.0)
